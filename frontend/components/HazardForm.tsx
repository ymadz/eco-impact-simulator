...
interface HazardFormProps {
  onSubmit?: (data: HazardReport) => void;
}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {Object.entries(HAZARD_TYPES).map(([key, hazard]) => (
              <button
                ...
                className={`p-6 rounded-xl border-2 transition-all hover:border-green-500 hover:bg-green-50 ${hazardType === key ? 'border-green-500 bg-green-50' : 'border-gray-200'
                  }`}
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(SEVERITY_LEVELS).map(([key, level]) => (
                  <button
                    ...
                    className={`p-3 rounded-lg border-2 text-left transition-all ${severity === key ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'
                      }`}
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  {question.options.map((option) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-full border-2 text-sm transition-all ${responses[question.id] === option
